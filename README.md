...
It is also worth nothing that, in addition to the additional privileges we gave nsenter in the Kubernetes parameters, running as the root user within the container was required for this escape to work. Using Falco to alert on that when it happens (which as you can see it does by default with the rules in the Helm chart), as well as perhaps having OPA Gatekeeker block that so it isn't even possible, you can iterate through your environment to get all of your containers running both as non-root and without privileges to really elevate your security posture. 
