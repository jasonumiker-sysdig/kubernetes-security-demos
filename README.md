...
1. `crictl exec -it (copy/paste ID - far left of last output) /bin/sh` And now I am in that container interactively
...
1. `crictl stop (copy/paste ID) && crictl rm (copy/paste ID)` and I can bypass Kubernetes' API and as the container runtime to stop/delete containers/Pods Kubernetes has launched on this Node as well
1. `exit` to leave the container
...
1. `./example-curls.sh` - to run these example curls which will:
    ...
    1. Download as script that, in turn, tries to run some cyrpto mining within our Pod
...
1. `cd ../demos` - Okay now lets try our nsenter again
...
1. `cd ../opa-gatekeeper/policies/constraint-templates/` then `cat` the various files in here to look at the policies (called constraint-templates) that made that possible
1. `cd ../constraints` then `cat` the various files in here - while the previous constraint-templates are the polices constraints say when - and when not to - apply those policies. So constraint-templates are not enforced until a constraint says where and/or where not to apply it.
1. `cd ../..`
...
While there are many tools available for this, Docker has a partnership with Snyk and built this functionality right into the Docker CLI you most likely are using to build your containers. As part of this there is a free tier. It does require you to be logged in to Docker Hub to use it.

1. Run `cd demos/security-playground`
